...
/// 启动前业务逻辑
pub async fn after_startup(main_pool: &Arc<SqlitePool>, log_pool: &Arc<SqlitePool>) -> Result<()> {
    // 启动每日任务
    daily::start_daily_tasks(Arc::clone(main_pool), Arc::clone(log_pool));

    // 打印数据库表和数据量
    let tables = sqlx::query("SELECT name FROM sqlite_master WHERE type='table'")
        .fetch_all(main_pool.as_ref())
        .await?;
    ...
    // 启动 Caddy 日志导入任务 (5s)
    caddy::start_caddy_log_task(Arc::clone(log_pool));
